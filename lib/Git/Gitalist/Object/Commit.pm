...
method get_patch ( Maybe[MooseX::Types::Common::String::NonEmptySimpleStr] $parent_hash?,
                   Int $patch_count?) {
    ...
    if (defined $parent_hash) {
        ...
        #  otherwise: --root h
        push @cmd, length($self->parents) > 1
            ? '--cc' : '--root';
        push @cmd, $self->sha1;
    }
...
method diff ( Bool              :$patch?,
              MooseX::Types::Common::String::NonEmptySimpleStr :$parent?,
              MooseX::Types::Common::String::NonEmptySimpleStr :$filename?
            ) {
    $parent = $parent
        ? $parent
            : $self->parents <= 1
...
method blame ( MooseX::Types::Common::String::NonEmptySimpleStr $filename, MooseX::Types::Common::String::SimpleStr $sha1 ) {
